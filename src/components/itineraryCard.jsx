...
export default function ItineraryCard({ itinerary }) {
  ...
  <div className='flex justify-between relative'>
    <button className="text-xl transition-colors text-gray-400">
      <FaHeart />
    </button>
    ...
    <h2 className="font-semibold text-base">{itinerary.name}</h2>
        ...
        <div className="flex flex-wrap gap-1 text-xs text-blue-500">
          {itinerary.hashtags?.map((tag, i) => (
            <span key={i} className="bg-blue-100 px-2 py-1 rounded-full">#{tag}</span>
          ))}
        </div>
      </div>
      ...
      {expanded && (
        <div className="px-4 pb-4 text-sm text-gray-600 border-t">
          <p className="pt-4">
            Información adicional, descripción extendida o detalles que se muestran al expandir.
          </p>
        </div>
      )}
    </div>
